...
type NodeType = 'department' | 'subsidiary' | 'project';
...
interface NodeData {
  ...
  color: string;
  progress?: number;
  status?: 'processing' | 'completed' | 'delayed';
  description?: string;
  docs?: Doc[];
  participants?: string[];
  milestones?: Milestone[]; // 项目特有的里程碑
}

// --- 数据定义 ---

const BASE_DEPARTMENTS: NodeData[] = [
...
// 辅助生成通用里程碑
const GENERIC_MILESTONES: Milestone[] = [
...
const ThreeGraph = ({ 
  ...
  data: { departments: NodeData[], projects: NodeData[] }
}) => {
  ...
  const targetQuaternionRef = useRef<any>(null);
  ...
  useEffect(() => {
    ...
    const deptPositions: {[key: string]: any} = {};

    const createTextSprite = (message: string, color: string, fontSize: number) => {
    ...
    data.projects.forEach((proj) => {
      ...
      proj.participants?.forEach(pid => {
        if (deptPositions[pid]) {
          ...
          // Changed color to white (0xffffff) and opacity to 0.6 for significantly better visibility
  ...
  useEffect(() => {
     ...
     sceneRef.current.children.forEach((obj: any) => {
        if (obj.type === 'Line') {
            ...
            if (selectedNodeId) {
                ...
                obj.material.opacity = 0.6; // Increased from 0.15 to 0.6 for better visibility
                ...
                <div className="space-y-4">
                  <div className="bg-slate-800/80 p-4 rounded-lg border border-slate-700">
                    ...
                    <p className="text-sm text-slate-400">
                      该部门共有 <strong className="text-white mx-1">
